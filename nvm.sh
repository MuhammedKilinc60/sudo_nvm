...
nvm_ls_remote() {
    ...
    local VERSIONS
    if [ "$PATTERN" ]; then
        if echo "${PATTERN}" | grep -v '^v' >/dev/null; then
            PATTERN="v$PATTERN"
        fi
    else
        PATTERN=".*"
    fi
    VERSIONS=$(curl -sL https://nodejs.org/dist/index.json | grep -oP 'v\d+\.\d+\.\d+' | grep -w "${PATTERN}" | sort -t. -u -k 1.2,1n -k 2,2n -k 3,3n)
    if [ ! "$VERSIONS" ]; then
        echo "N/A"
        return
    fi
